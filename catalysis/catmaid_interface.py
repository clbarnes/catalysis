...
class CatmaidDataInterface:
    ...
    def url_to_point(
        self, xyz, node_skeleton_ids=None, tool="tracingtool", zoomlevel=0, show=True
    ):
        """
        ...
        if show == True:
            display(
                HTML('<a href="{}">{}</a>'.format(out_url, "Link to point " + str(xyz)))
            )
        return out_url
        ...
        for neuron_id in id_list:
            ...
            display(
                HTML(
                    '<a href="{}">{}</a>'.format(
                        self.url_to_point(
                            xyz, (root_info["root_id"], neuron_id), show=False
                        ),
                        names[str(neuron_id)],
                    )
                )
    ...
    def node_location(self, node_id):
        """ """
        url = "/{}/treenodes/{}/compact-detail".format(
            self.CatmaidClient.project_id, node_id
        )
    ...
    def get_neuron_name(self, skeleton_id):
        ...
        str
            Name of neuron
        """
        d = self.CatmaidClient.get(
            "/{}/skeleton/{}/neuronname".format(
                self.CatmaidClient.project_id, skeleton_id
            )
        )
    ...
    def tag_query_for_skeleton(self, id, tag_query):
        ...
        data = {
            "treenode_id": self.root_for_skeleton(id)["root_id"],
    ...
    def get_closed_ends(self, id):
        ...
        """
        tag_query = (
            "^ends$|^soma$|^uncertain end$|^uncertain continuation$|^not a branch$"
        )
    ...
    def _get_compact_skeleton(self, skeleton_id, withtags):
        ...
        if withtags:
            ...
            tag_flag = 0
        url = "/{}/{}/1/{}/compact-skeleton".format(
            self.CatmaidClient.project_id, skeleton_id, tag_flag
        )
        return self.CatmaidClient.get(url)
    ...
    def skeleton_statistics(self, skeleton_id):
        ...
        """
        url = "{}/skeleton/{}/statistics".format(
            self.CatmaidClient.project_id, skeleton_id
        )
        d = self.CatmaidClient.get(url)
        return d
    ...
    def get_cable_length(self, id_list, num_per_call=50):
        ...
        id_list_chunked = [
            id_list[ii : ii + num_per_call]
            for ii in range(0, len(id_list), num_per_call)
        ]
        lens = {}
    ...
    def get_annotations(self):
        ...
        anno_dict = {
            item["name"]: item["id"] for item in all_annotations["annotations"]
        }
        return anno_dict
    ...
    def get_ids_from_annotations(
        self, annotation_id_list, skip_annotations=None, flatten=False
    ):
        """
        ...
        annotation_id_list = self.parse_annotation_list(
            annotation_id_list, output="ids"
        )
    ...
    def in_bounding_box(
        self, x_min, x_max, y_min, y_max, z_min, z_max, min_nodes=None, min_cable=None
    ):
        """
    ...
    def get_connected_skeletons(
        self,
        id_list,
        include_presynaptic=True,
        include_postsynaptic=True,
        presynaptic_cutoff=0,
        postsynaptic_cutoff=0,
        node_cutoff=0,
    ):
        """
    ...
    def get_connected_skeletons_flat(
        self,
        id_list,
        include_presynaptic=True,
        include_postsynaptic=True,
        ...
        conn_sk = self.get_connected_skeletons(
            id_list,
            include_presynaptic,
            include_postsynaptic,
            presynaptic_cutoff,
            postsynaptic_cutoff,
            node_cutoff,
        )
    ...
    def get_connectivity_graph_edge_list(self, id_list):
        ...
        """
        url = "/{}/skeletons/confidence-compartment-subgraph".format(
    ...
    def get_network_neighborhood(
        ...
        id_list,
        number_of_hops,
        pre_minimum_synapses=1,
        post_minimum_synapses=1,
        include_names=False,
    ):
        """
    ...
    def find_intermediate_neurons(
        ...
        for o_hops in range(1, number_of_hops - 1):
            ...
            origin_neighborhood = set(
                self.get_network_neighborhood(
                    origin_ids,
                    o_hops,
    ...
    def get_landmarks(self, with_locations=True):
        ...
        url = "/{}/landmarks/?with_locations={}".format(
            self.CatmaidClient.project_id, str(with_locations).lower()
        )
    ...
    def reroot_neurons_to_soma(self, id_list):
        ...
        for skid in id_list:
            ...
            if len(dsoma) == 0:
                print(
                    "Skipping {} for want of a soma. Fix at the link below:".format(
                        skid
                    )
                )
                ...
                print(
                    "Skipping {} which has too many somata. Fix at the link below:".format(
                        skid
                    )
                )
                self.url_to_neurons([skid])
    ...
    def export_json_for_selection_widget(
        self, filename, skids, color=None, opacity=None
    ):
        if color is None:
        ...
        for skid in skids:
            dat.append(
                dict(
                    skeleton_id=skid,
                    color=cl.to_hex(color[skid]),
                    opacity=opacity[skid],
                )
            )
