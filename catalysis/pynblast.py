...
def nblast_neuron_pair(
    ...
    """
    d_and_udotv = neuron_comparison_nblast_components(
        nrn_q, nrn_t, resample_distance=resample_distance, num_nn=num_nn
    )
    S_a = nblast_dist_fun(d_and_udotv, score_lookup)
    if bidirectional:
        d_and_udotv_b = neuron_comparison_nblast_components(
            nrn_t, nrn_q, resample_distance=resample_distance, num_nn=num_nn
        )
...
def point_cloud_nblast(nrn_q_dotprop, nrn_t_dotprop, score_lookup, max_proximity=None):
    ...
    """
    d_and_udotv = neuron_comparison_nblast_components(
        nrn_q_dotprop, nrn_t_dotprop, as_dotprop=True, max_proximity=max_proximity
    )
...
def max_blast_score(L, score_lookup):
    ...
    """
    d_and_udotv = np.transpose(np.vstack((np.zeros(L), np.ones(L))))
...
def neuron_to_dotprop(nrn, resample_distance=1000, num_nn=5, min_strahler=None):
    ...
    for ind, pth in enumerate(pths):
        if len(pth) > 0:
            dp.append(
                dotprop_path(
                    [nrn.nodeloc[nid] for nid in pth],
                    resample_distance=resample_distance,
                    num_nn=num_nn,
                )
            )
...
def dotprop_path(xyz, resample_distance, num_nn=5):
    ...
    for n in xyzi:
        nnxyz = xyzi[
            dist_tree.query(n, query_nn)[1]
        ]  # +1 because this will return original node also.
...
def neuron_comparison_nblast_components(
    source_nrn,
    target_nrn,
    resample_distance=1000,
    num_nn=5,
    ...
    if as_dotprop:
        ...
        source_dp = neuron_to_dotprop(
            source_nrn, resample_distance=resample_distance, num_nn=num_nn
        )
        target_dp = neuron_to_dotprop(
            target_nrn, resample_distance=resample_distance, num_nn=num_nn
        )
    udotv = []
    ...
    if min(ds[0]) > max_proximity:
        ...
        d_and_udotv[:, 0] = ds[0]
        d_and_udotv[:, 1] = np.einsum(
            "ij,ij->i", source_dp[:, 3:], target_dp[ds[1], 3:]
        )
    else:
        d_and_udotv = np.zeros((len(ds[0]), 2))
        d_and_udotv[:, 0] = ds[0]
...
def nblast_neurons(
    ...
    if as_dotprop:
        ...
        neuron_to_dotprop_cond = partial(
            neuron_to_dotprop,
            resample_distance=resample_distance,
        ...
        nrns_q_dotprop = {
            nrn.id: nrns_q_dotprop_list[ii] for ii, nrn in enumerate(nrns_q)
        }
    ...
    if nrns_t is None:
        ...
        for nrn in nrns_t:
            if as_dotprop:
                ...
                nrns_t_dotprop = {
                    nrn.id: nrns_t_dotprop_list[ii] for ii, nrn in enumerate(nrns_t)
                }
    ...
    for nrn_q in nrns_q:
        ...
        for nrn_t in nrns_t:
            ...
            if as_dotprop:
                args.append(
                    (
                        nrns_q_dotprop[nrn_q],
                        nrns_t_dotprop[nrn_t],
                        score_lookup,
                        resample_distance,
                        num_nn,
                        normalize,
                ...
                args.append(
                    (
                        nrns_q_dotprop[nrn_q.id],
                        nrns_t_dotprop[nrn_t.id],
                        score_lookup,
                        resample_distance,
                        num_nn,
                        normalize,
    ...
    pool.close()
    df = pd.DataFrame(
        {"Queries": queries, "Targets": targets, "S": similarities}
    ).reindex(columns=["Queries", "Targets", "S"])
    return df
...
def soma_distance(nrns_q, nrns_t=None):
    ...
    df = pd.DataFrame(
        {"Queries": queries, "Targets": targets, "soma_distance": soma_distances}
    ).reindex(columns=["Queries", "Targets", "soma_distance"])
    return df
...
def exact_nblast(
    ...
    Stq = nblast_neurons(
        ...
        normalize=True,
        as_dotprop=as_dotprop,
    ).pivot(index="Targets", columns="Queries", values="S")
    Stq[Stq < 0] = 0
    Sqt[Sqt < 0] = 0
...
def max_match_similarity(
    Sb, min_similarity=0.4, enforce_match=None, enforce_match_val=1
):
    ...
    match_df = (
        pd.DataFrame(
            {
                "Query_name": name_q,
                "Query_id": match_ids_q,
                "Target_name": name_t,
                "Target_id": match_ids_t,
                "S": match_val,
            }
        )
    ...
    return match_df.reindex(
        ["S", "Query_name", "Query_id", "Target_name", "Target_id"], axis=1
    )


def compare_partners(
    score_lookup,
    nrn_q,
    nrn_t,
    CatmaidInterface,
    ...
    partner_nrns_q = cat.NeuronList.from_id_list(
        id_list=partner_ids_all_q[0:ntop_q, 0],
    ...
    partner_ids_all_t = nrn_t.synaptic_partners(
        connection_type=connection_type,
        min_synapses=min_synapses,
        normalized=normalized,
    )
    partner_nrns_t = cat.NeuronList.from_id_list(
        id_list=partner_ids_all_t[:, 0],
        CatmaidInterface=CatmaidInterface,
        with_tags=True,
        with_annotations=False,
    )
    ...
    if return_neurons:
        ...
        out["partners_t"] = partner_nrns_t_transformed.slice_by_id(
            matching["Target_id"]
        )
...
def paired_connectivity_vector(
    nrns, pair_map, is_mirrored=False, normalize_weights=False
):
    """
    ...
    for skid in conn_sk["postsynaptic"]:
        ...
        for pid in rel_partner_ids:
            if normalize_weights:
                print(
                    conn_sk["postsynaptic"][skid][pid],
                    num_inputs_post_dict[pid],
                    conn_sk["postsynaptic"][skid][pid]
                    / (num_inputs_post_dict[pid] + 0.0),
                )
                vec_out[pair_map["skid_to_ind"][pid]] = conn_sk["postsynaptic"][skid][
                    pid
                ] / (num_inputs_post_dict[pid] + 0.0)
            else:
                vec_out[pair_map["skid_to_ind"][pid]] = conn_sk["postsynaptic"][skid][
...
def paired_connectivity_prob(nrns_q, nrns_t, pair_map, conn_stats, is_mirrored=True):
    """
    ...
    """
    pv_q_in, pv_q_out = paired_connectivity_vector(
        nrns_q, pair_map=pair_map, normalize_weights=False
    )
    pv_t_in, pv_t_out = paired_connectivity_vector(
        nrns_t, pair_map=pair_map, is_mirrored=is_mirrored, normalize_weights=False
    )
    ...
    for qid in nrns_q.ids():
        for tid in nrns_t.ids():
            ...
            pre_prob.append(
                np.sum(
                    match_prob_conn_log_ratio(
                        pv_q_in[qid].astype(int), pv_t_in[tid].astype(int), conn_stats
                    )
                )
            )
            post_prob.append(
                np.sum(
                    match_prob_conn_log_ratio(
                        pv_q_out[qid].astype(int), pv_t_out[tid].astype(int), conn_stats
                    )
                )
            )
    return pd.DataFrame(
        {"Queries": Qids, "Targets": Tids, "pre_prob": pre_prob, "post_prob": post_prob}
    )


def match_likelihood_ratio(
    nrns_q,
    nrns_t,
    pair_map,
    conn_stats,
    ...
    P_con = paired_connectivity_prob(
        nrns_q, nrns_t, pair_map, conn_stats, is_mirrored=is_mirrored
    )

    # Put all the computations together into one dataframe.
    Stot = pd.merge(
        Sqt.rename(index=str, columns={"S": "Sqt"}),
        Stq.rename(
            index=str, columns={"Queries": "Targets", "Targets": "Queries", "S": "Stq"}
        ),
        on=["Queries", "Targets"],
    )
    ...
    Stot["P_morph_log_ratio"] = match_prob_morpho_log_ratio(
        Stot.Sbid, Stot.soma_distance, morpho_stats
    )
...
def match_prob_conn_log_ratio(w1, w2, conn_stats):
    if (
        w1 > np.shape(conn_stats["log_ratio"])[0] - 1
        or w2 > np.shape(conn_stats["log_ratio"])[1] - 1
    ):
...
def match_prob_conn_normalized_log_ratio(w1, w2, conn_stats):
    if w1 > 0 or w2 > 0:
        ...
        var_param_ind = int(
            np.where(
                (wmean < conn_stats["percentiles"][1:])
                & (wmean >= conn_stats["percentiles"][:-1])
            )[0]
        )
        p_match = sp.stats.lognorm.pdf(
            wmean, *conn_stats["freq_param"]
        ) * sp.stats.gamma.pdf(np.abs(w1 - w2), *conn_stats["var_param"][var_param_ind])
        p_nonmatch = sp.stats.lognorm.pdf(
            w1, *conn_stats["freq_param"]
        ) * sp.stats.lognorm.pdf(w2, *conn_stats["freq_param"])
        return np.log2(p_match) - np.log2(p_nonmatch)
    else:
        return 0.0


def compare_partners_prob(
    nrn_q,
    nrn_t,
    connection_type,
    CatmaidInterface,
