...
class NeuronList:
    def __init__(
        self, NeuronObjDict, CatmaidInterface=None, export_date=None, project_name=None
    ):
        """
    ...
    def slice_by_id(self, id_list):
        ...
        neurons_sampled = {
            id: self.neurons[id] for id in id_list if id in self.neurons.keys()
        }
    ...
    def from_id_list(
        ...
        neurons = {
            id: NeuronObj.from_catmaid_data(
                id, d["skeletons"][str(id)], CatmaidInterface=CatmaidInterface
            )
            for id in id_list
            if id is not None
        }
        return cls(
            ...
            export_date=datetime.datetime.now().strftime("%c"),
        )
    ...
    def from_annotations(
        ...
        """
        id_list = CatmaidInterface.get_ids_from_annotations(
            annotation_list, flatten=True
        )
        if skip_annotations is not None:
            id_to_skip = CatmaidInterface.get_ids_from_annotations(
                skip_annotations, flatten=True
            )
            id_list = list(set(id_list).difference(set(id_to_skip)))

        return cls.from_id_list(
            id_list,
            CatmaidInterface,
            with_tags=with_tags,
            with_annotations=with_annotations,
    ...
    def from_msc_json_import(cls, filename):
        ...
        neurons = {
            neuron_info["id"]: NeuronObj(neuron_info)
            for neuron_info in neurons_info_list
        }
        ...
        for id_str in project_data["neuron_info"]:
            ...
            if neuron_data["morphology"]["node_property_list"] is not None:
                if "radius" in neuron_data["morphology"]["node_property_list"]:
                    rad_ind = neuron_data["morphology"]["node_property_list"].index(
                        "radius"
                    )
                    for row in neuron_data["morphology"]["node_properties"]:
                        if row[rad_ind] > 0:
            ...
            connector_data_post, connector_locs_post = self._dict_to_connector_data(
                conn_post_ids, project_data
            )

            conn_pre_ids = [row[1] for row in neuron_data["connectivity_pre"]]
            connector_data_pre, connector_locs_pre = self._dict_to_connector_data(
                conn_pre_ids, project_data
            )

            neuron_info["inputs"] = InputSynapseListObj(
                connector_data_post, connector_locs_post, neuron_info["id"]
            )
            neuron_info["outputs"] = OutputSynapseListObj(
                connector_data_pre, connector_locs_pre, neuron_info["id"]
            )
    ...
    def _dict_to_connector_data(self, connector_list, project_data):
        ...
        for conn_id in connector_list:
            ...
            conn_dict = {}
            conn_dict["postsynaptic_to"] = [
                row[1] for row in conn_dat["connectivity_post"]
            ]
            conn_dict["postsynaptic_to_node"] = [
                row[0] for row in conn_dat["connectivity_post"]
            ]
    ...
    def save_to_pickle(self, filename):
        ...
        nrn_copy.CatmaidInterface = (
            None  # Strip out user-specific catmaid interface details
        )
    ...
    def annotations_from_neurons(self):
        ...
        for anno_id in anno_dat["annotations"]:
            anno_dict[int(anno_id)] = {
                "str": anno_dat["annotations"][anno_id],
                "skids": [],
            }
                ...
                for targ in nrn.outputs.target_ids[conn_id]:
                    if targ in ids:
                        if input_normalized is True:
                            A[skid_to_ind[targ], skid_to_ind[nrn.id]] += (
                                1.0 / self.neurons[targ].inputs.num()
                            )
                        else:
    ...
    def export_adjacency_matrix(
        self, filename, index="id", input_normalized=False, delimiter=","
    ):
        """
        ...
        with open(filename, "w") as fid:
            A, skid_to_ind, ind_to_skid = self.get_adjacency_matrix(
    ...
    def group_adjacency_matrix(self, groups, func=np.sum, input_normalized=False):
        ...
        """
        A, skid_to_ind, ind_to_skid = self.get_adjacency_matrix(
            neurons, input_normalized=input_normalized
        )
        Agr = np.zeros((len(groups), len(groups)))
        for ii, grp_post in enumerate(groups):
            for jj, grp_pre in enumerate(groups):
                Ared = A[[skid_to_ind[post] for post in grp_post], :][
                    :, [skid_to_ind[pre] for pre in grp_pre]
                ]
                Agr[ii, jj] = func(Ared)
        return Agr
    ...
    def msc_json_export(
        ...
        neuron_info = self._msc_neuron_info(
            morpho_columns, topo_columns, annotation_id_whitelist
        )
    ...
    def _msc_connector_info(self):
        ...
        for neuron in self.neurons:
            connector_list = (
                connector_list + neuron.inputs.conn_ids + neuron.outputs.conn_ids
            )
            connector_location_list = {
                **connector_location_list,
                **neuron.inputs.locs,
        ...
        for conn in conn_dat:
            ...
            connector_info[conn[0]]["node_properties"] = [
                conn[0]
            ] + connector_location_list[conn[0]]
            connector_info[conn[0]]["topology"] = None
            connector_info[conn[0]]["connectivity_pre"] = (
                conn[1]["presynaptic_to_node"],
                conn[1]["presynaptic_to"],
            )
            connector_info[conn[0]]["connectivity_post"] = list(
                zip(conn[1]["postsynaptic_to_node"], conn[1]["postsynaptic_to"])
            )
        return connector_info

    def _msc_neuron_info(
        self, morpho_columns=None, topo_columns=None, annotation_id_whitelist=None
    ):
        neuron_info = {}
        for nrn in self.neurons:
            ...
            if annotation_id_whitelist is None:
                ...
                annotation_ids = [
                    key for key in nrn_annos.keys() if key in annotation_id_whitelist
                ]
            neuron_info[nid]["neuron_annotations"] = list(
                annotation_ids
            )  # A list of annotation ids

            # Morphology is everything related to nodes
            if (
                morpho_columns is not None
            ):  # Morpho columns needs to be dict of dicts. First, property name, second node id.
                node_properties = [
                    [id]
                    + neuron.nodeloc[id]
                    + [morpho_columns[prop][id] for prop in morpho_columns]
                    for id in neuron.nodeloc
                ]
            ...
            neuron_info[nid]["morphology"] = {
                "node_properties": node_properties,
                "node_property_list": node_property_list,
            }

            # Topology is everything related to how nodes connect within a neuron
            topology = {}
            edge_list = [
                [node_id, neuron.nodeparent[node_id]] for node_id in neuron.nodeparent
            ]
            ...
            for conn_id in neuron.outputs.from_node_ids:
                neuron_info[nid]["connectivity_pre"].append(
                    (neuron.outputs.from_node_ids[conn_id], conn_id)
                )
    ...
    def _msc_annotation_info(self, annotation_id_whitelist=None):
        ...
        if annotation_id_whitelist is not None:
            anno_id_list = [
                anno for anno in anno_dat if anno in annotation_id_whitelist
            ]
        else:
            anno_id_list = anno_dat.keys()
        for anno in anno_id_list:
            annotation_info[anno] = {
                "anno_string": anno_dat[anno]["str"],
                "ids": anno_dat[anno]["skids"],
            }
...
class InputSynapseListObj(SynapseListObj):
    def __init__(self, conndata, locs, self_id):
        ...
        for dat in conndata:
            for skid, nid in zip(
                dat[1]["postsynaptic_to"], dat[1]["postsynaptic_to_node"]
            ):
                if skid == self_id:
...
class OutputSynapseListObj(SynapseListObj):
    def __init__(self, conndata, locs, self_id):
        ...
        self.target_node_ids = {
            dat[0]: dat[1]["postsynaptic_to_node"] for dat in conndata
        }
...
class NeuronObj:
    def __init__(self, neuron_info_dict):
        ...
        temp_root = [
            nid
            for nid in neuron_info_dict["nodeparent"]
            if neuron_info_dict["nodeparent"][nid] is None
        ]
        self.root = temp_root[0]

        self.A = sparse.dok_matrix(
            (len(neuron_info_dict["nodeloc"]), len(neuron_info_dict["nodeloc"])),
            dtype=np.float32,
        )
        ...
        for key in neuron_info_dict["nodeparent"].keys():
            if neuron_info_dict["nodeparent"][key] is not None:
                self.A[
                    ...
                    neuron_info_dict["nodeloc"][key],
                    neuron_info_dict["nodeloc"][neuron_info_dict["nodeparent"][key]],
                )
                self.Ab[
                    self.node2ind[key],
                    self.node2ind[neuron_info_dict["nodeparent"][key]],
    ...
    def from_catmaid_data(cls, skid, skdata, CatmaidInterface):
        ...
        pre_conn_locs = {
            conn_row[1]: conn_row[3:6]
            for conn_row in skdata[1]
            if conn_row[1] in pre_conn_ids
        }
        post_conn_locs = {
            conn_row[1]: conn_row[3:6]
            for conn_row in skdata[1]
            if conn_row[1] in post_conn_ids
        }

        neuron_info_dict["inputs"] = InputSynapseListObj(
            CatmaidInterface.get_connector_data(post_conn_ids), post_conn_locs, skid
        )
        neuron_info_dict["outputs"] = OutputSynapseListObj(
            CatmaidInterface.get_connector_data(pre_conn_ids), pre_conn_locs, skid
        )

        neuron_info_dict["annotations"] = CatmaidInterface.get_annotations_for_objects(
            [skid]
        )
    ...
    @classmethod
    def from_catmaid(
        cls, skid, CatmaidInterface, with_tags=True, max_neurons_per_post=100
    ):
        neuron_info_dict = {}
        neuron_info_dict["id"] = skid

        skdata = CatmaidInterface.get_skeleton_data(
            skid, with_tags=with_tags, max_neurons_per_post=max_neurons_per_post
        )
    ...
    def strahler_filter(self, min_sn):
        ...
        new_tags = {
            tag: list(set(self.tags[tag]).difference(set(inds_to_remove)))
        ...
        nni["nodeloc"] = {
            nid: self.nodeloc[nid] for nid in self.nodeloc if nid not in inds_to_remove
        }

        nni["nodeparent"] = {
            nid: self.nodeparent[nid]
            for nid in self.nodeparent
            if nid not in inds_to_remove
        }
        nni["radius"] = {
            nid: self.radius[nid] for nid in self.radius if nid not in inds_to_remove
        }

        input_obj = self.inputs
        input_obj.target_node_ids = {
            cid: list(
                set(self.inputs.target_node_ids[cid]).difference(set(inds_to_remove))
            )
            for cid in self.inputs.target_node_ids
        }
    ...
    def minimal_paths(self):
        ...
        ends_sorted = [
            ids_end[ind]
            for ind in np.argsort(D[[self.node2ind[id] for id in ids_end]])[::-1]
        ]
        ...
        min_paths_sorted = [
            min_paths[ind] for ind in np.argsort(list(map(len, min_paths)))[::-1]
        ]
    ...
    def synaptic_partners(self, connection_type, min_synapses=0, normalized=False):
        ...
        if connection_type == "presynaptic":
            ids, syns = np.unique(
                list(filter(None, self.inputs.from_ids.values())), return_counts=True
            )
        elif connection_type == "postsynaptic":
            ids, syns = np.unique(
                [
                    x
                    for x in chain.from_iterable(self.outputs.target_ids.values())
                    if x is not None
                ],
                return_counts=True,
            )
...
def synaptic_partner_tables(
    neurons, include_presynaptic=True, include_postsynaptic=True, use_name=False
):
    """
