...
def open_ends(nrn, include_uncertain=True):
    ...
    if include_uncertain:
        ...
        blessed_tags = ["ends", "soma", "not a branch"]
...
def property_summary_exact(nrns):
    ...
    if nrns.CatmaidInterface is not None:
        ...
        rev_frac = [
            (review_status[str(id)][1] + 0.0) / (review_status[str(id)][0] + 0.0)
            for id in ids
        ]
...
def completion_categories(
    property_summary=None, completeness_threshold=0.97, review_threshold=0.97
):
    """
    ...
    if property_summary is not None:
        for id in property_summary.index.values:
            if property_summary.loc[id]["node_count"] == 1:
                ...
                if (
                    property_summary.loc[id]["fraction_open_ends"]
                    <= 1 - completeness_threshold
                ):
...
def category_summary(categories, syn_df=None, as_df=True, nans=False):
    ...
    ids_cat = set(
        [
            item
            for sublist in [categories[cat] for cat in categories]
            for item in sublist
            if item is not None
        ]
    )
    ...
    if as_df:
        return pd.DataFrame({"Synapses": cat_by_syn, "Number": cat_by_num}).reindex(
            [
                "Untraced",
                "Incomplete fragment",
                "Incomplete neuron",
...
def category_summary_from_neurons(
    nrns,
    ...
    input_df, output_df = na.synaptic_partner_tables(
        nrns,
        include_presynaptic=include_presynaptic,
        include_postsynaptic=include_postsynaptic,
    )
    if include_presynaptic:
        ...
        if estimate_partner_completion:
            input_neurons = na.NeuronList.from_id_list(
                pre_ids,
                nrns.CatmaidInterface,
                max_neurons_per_post=max_neurons_per_post,
            )
            input_cats = completion_categories(property_summary_exact(input_neurons))
        else:
            input_cats = completion_categories(
                property_summary_estimated(pre_ids, nrns.CatmaidInterface)
            )
    ...
    if include_postsynaptic:
        ...
        if estimate_partner_completion:
            output_neurons = na.NeuronList.from_id_list(
                post_ids,
                nrns.CatmaidInterface,
                max_neurons_per_post=max_neurons_per_post,
            )
            ...
            output_cats = completion_categories(
                property_summary_estimated(post_ids, nrns.CatmaidInterface)
            )
...
def completeness_report(
    ...
    id_list = list(
        set(id_list + CatmaidInterface.get_ids_from_annotations(annos, flatten=True))
    )
    nrns = na.NeuronList.from_id_list(
        id_list, CatmaidInterface, max_neurons_per_post=max_neurons_per_post
    )
    cats = category_summary_from_neurons(
        nrns,
        include_presynaptic=include_presynaptic,
        include_postsynaptic=include_postsynaptic,
    )
...
def report_from_summary(category_summary, complete_categories=None, anno_name=None):
    ...
    (
        ...
        category_summary["Base"], complete_categories=complete_categories
    )
    (
        input_complete,
        input_inc,
        ...
        category_summary["Inputs"], complete_categories=complete_categories
    )
    (
        output_complete,
        output_inc,
        ...
        category_summary["Outputs"], complete_categories=complete_categories
    )

    num_master = {
        anno_name: base_complete,
        "Inputs": input_complete,
        "Outputs": output_complete,
    }

    inc_master = {anno_name: base_inc, "Inputs": input_inc, "Outputs": output_inc}

    frac_master = {anno_name: base_frac, "Inputs": input_frac, "Outputs": output_frac}

    num_master_syn = {
        anno_name: base_complete_syn,
        "Inputs": input_complete_syn,
        "Outputs": output_complete_syn,
    }

    frac_master_syn = {
        anno_name: base_complete_syn / sum(category_summary["Base"].Synapses),
    ...
    return pd.DataFrame(
        {
            "Number Complete": num_master,
            "Number Incomplete": inc_master,
            "Fraction Complete": frac_master,
            "Synapses Complete": num_master_syn,
            "Fraction Synapses Complete": frac_master_syn,
        }
    )
...
def match_groups(
    id_list1, id_list2, match_via, CatmaidInterface, anno_reference="names"
):
    """
    ...
    """
    annos_with_meta = set(
        CatmaidInterface.get_annotations_from_meta_annotations(match_via, flatten=True)
    )
    annos1 = {
        id: set(CatmaidInterface.get_annotations_for_objects([id])).intersection(
            annos_with_meta
        )
        for id in id_list1
    }
...
def match_report(
    ...
    if skip_annos is not None:
        ids_to_skip = CatmaidInterface.get_ids_from_annotations(
            skip_annos, flatten=True
        )
        ...
        report = pd.DataFrame(
            {
                name1: match_report1,
                name2: match_report2,
                name1 + "_complete": match_completed1,
                name2 + "_complete": match_completed2,
            }
        )
    else:
...
def _match_completed(
    ...
    for lin in match_report:
        ...
        for skid in relids:
            if (
                ps[ps.index == skid]["is_fragment"].bool() is False
                and (ps[ps.index == skid]["fraction_open_ends"] < min_open_ends).bool()
            ):
...
def match_report_from_annos(
    ...
    """
    return match_report(
        CatmaidInterface.get_ids_from_annotations(anno1, flatten=True),
...
def report_from_annotation_list(anno_list, CatmaidInterface):
    ...
    """
    meta_rep = pd.DataFrame(
        columns=[
            "Number Complete",
            "Number Incomplete",
            "Fraction Complete",
            "Synapses Complete",
            "Fraction Synapses Complete",
        ]
    )
...
def assert_pair(nrn_ids, pair_meta, CatmaidInterface):
    ...
    all_pair_annos = set(
        CatmaidInterface.get_annotations_from_meta_annotations(pair_meta, flatten=True)
    )
    nrn_annos = set(CatmaidInterface.get_annotations_for_objects(nrn_ids))

    if len(all_pair_annos.intersection(nrn_annos)) > 0:
        for nrn_id in nrn_ids:
            specific_nrn_annos = set(
                CatmaidInterface.get_annotations_for_objects([nrn_id])
            )
        ...
        if len(nrn_ids) < 4:
            ...
            pair_name = (
                "hemilateral_pair"
                + "".join(["_{}".format(nid) for nid in sorted(nrn_ids[0:4])])
                + "_etc"
            )

        d = CatmaidInterface.add_annotation(
            annotation_list=[pair_name], id_list=nrn_ids, meta_list=pair_meta
        )
...
def is_matched(nrn_ids, pair_meta, CatmaidInterface):
    ...
    """
    annos_with_meta = CatmaidInterface.get_annotations_from_meta_annotations(
        pair_meta, flatten=True
    )
    ids_matched = CatmaidInterface.get_ids_from_annotations(
        annos_with_meta, flatten=True
    )
    has_match = {}
    for skid in nrn_ids:
...
def matched_complete_report(
    anno_list, pair_meta, CatmaidInterface, max_open_ends=0.03, min_nodes=500
):
    """ """
    anno_names = []
    left_incom = []
    ...
    for anno in anno_list:
        ...
        lincom, lincom_match, lcom, lcom_match = _match_category_helper(
            props_left, max_open_ends=max_open_ends, min_nodes=min_nodes
        )
        ...
        rincom, rincom_match, rcom, rcom_match = _match_category_helper(
            props_right, max_open_ends=max_open_ends, min_nodes=min_nodes
        )
...
def _match_category_helper(props, max_open_ends, min_nodes):
    incom = sum(
        np.logical_and(
            np.logical_or(
                props["fraction_open_ends"] >= max_open_ends,
...
def filter_complete(
    ...
    if sensory_exception:
        filt = (props["fraction_open_ends"] < max_open_ends) & (
...
def _paired_ids_matched(
    CatmaidInterface, match_report_df, pair_meta, max_open_ends=0.03, min_node_count=500
):
...
def _paired_ids_unmatched_ipsilateral(
    CatmaidInterface,
    id_list_1,
    id_list_2,
    pair_meta,
...
def _paired_ids_unmatched_contralateral(
    CatmaidInterface,
    id_list_1_comp,
    id_list_2_comp,
    match_report_df,
    ...
    for row in match_report_df.iterrows():
        if isinstance(row[1]["Group_1"], np.integer):
            ...
            match_ids_1 = row[1]["Group_1"]
        non_match_ids_1 = list(set(id_list_1_comp).difference(set(match_ids_1)))

        if isinstance(row[1]["Group_2"], np.integer):
            match_ids_2 = [row[1]["Group_2"]]
        else:
            match_ids_2 = row[1]["Group_2"]
        non_match_ids_2 = list(set(id_list_2_comp).difference(set(match_ids_2)))
...
def make_id_pairs(
    ...
    if type(id_list_1) is str:
        id_list_1 = CatmaidInterface.get_ids_from_annotations(id_list_1, flatten=True)
    if type(id_list_2) is str:
        id_list_2 = CatmaidInterface.get_ids_from_annotations(id_list_2, flatten=True)

    id_list_1_comp = filter_complete(
        id_list_1,
        CatmaidInterface,
    ...
    id_list_2_comp = filter_complete(
        id_list_2,
        CatmaidInterface,
        max_open_ends=max_open_ends,
        min_node_count=min_node_count,
    ...
    pairs_matched = _paired_ids_matched(
        CatmaidInterface,
        match_report_df,
        pair_meta,
        max_open_ends=max_open_ends,
    ...
    pairs_ipsi = _paired_ids_unmatched_ipsilateral(
        CatmaidInterface,
        id_list_1_comp,
        id_list_2_comp,
        pair_meta,
    ...
    pairs_contra = _paired_ids_unmatched_contralateral(
        CatmaidInterface,
        id_list_1_comp,
        id_list_2_comp,
        match_report_df,
...
def match_groups_arbitrary(
    list_of_id_lists, match_via, CatmaidInterface, nrns=None, anno_reference="names"
):
    """
    ...
    if nrns is None:
        for id_list in list_of_id_lists:
            annos_to_pair.append(
                {
                    nid: set(
                        CatmaidInterface.get_annotations_for_objects([nid])
                    ).intersection(annos_with_meta)
                    for nid in id_list
                }
            )
    else:
        for id_list in list_of_id_lists:
            annos_to_pair.append(
                {
                    nid: set(nrns[nid].annotations).intersection(annos_with_meta)
                    for nid in id_list
                }
            )
