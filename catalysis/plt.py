...
def neuronlist_data(
    ...
    for nrn in nrns:
        ...
        if with_synapses[nrn.id]:
            data.append(
                neuronal_synapse_data(
                    nrn, synapse_type="post", color=postsynaptic_color, markersize=2
                )
            )
            data.append(
                neuronal_synapse_data(
                    nrn, synapse_type="pre", color=presynaptic_color, markersize=2
                )
            )
...
def neuronal_morphology_data(nrn, color=(0, 0, 0), width=2):
    ...
    return go.Scatter3d(
        x=xyz[:, 0], y=xyz[:, 1], z=xyz[:, 2], line=line, mode="lines", name=nrn.name
    )
...
def neuronal_synapse_data(nrn, synapse_type, color, markersize):
    ...
    return go.Scatter3d(
        ...
        showlegend=False,
        mode="markers",
        marker=dict(symbol="circle", size=markersize, color=cl.to_hex(color)),
    )
...
def sphere_data(xyz0, color, radius=2000, npts=10):
    ...
    fig_prop = ff.create_trisurf(
        x=xs,
        y=ys,
        z=zs,
        simplices=simplices,
        colormap=[cl.to_hex(color), cl.to_hex(color)],
    )
...
def path_data(xyz, color=(0.3, 0.3, 0.3), line=None, width=2, name=None):
    ...
    return go.Scatter3d(
        x=xyz[:, 0], y=xyz[:, 1], z=xyz[:, 2], line=line, mode="lines", name=name
    )
...
def scatter_block(xyz, marker=None, color=None):
    ...
    return go.Scatter3d(
        x=xyz[:, 0], y=xyz[:, 1], z=xyz[:, 2], marker=marker, mode="markers"
    )


def match_report_plot_data(
    nrn_q, nrns_t, matches, color_q=(0.2, 0.2, 0.2), color_match="Set1"
):
    """
    Plot all neurons that were potential matches
    """
    nrn_q_data = neuronlist_data(nrn_q, with_synapses=False, color=color_q, width=4)
    match_nrns = []
    for row in matches[nrn_q.id].iterrows():
        ...
        try:
            ...
            continue
    nrn_t_data = neuronlist_data(
        match_nrns, with_synapses=False, color=color_match, width=2
    )
...
def connectivity_line_plot(A, plot_groups, colors, params=None):
    ...
    if "columnwidth" not in params:
        params[
            "columnwidth"
        ] = 10  # Width of the pre-to-post column units of number of nodes
    if "nodesize" not in params:
        params["nodesize"] = 10  # Sets node size
    if "linescaling" not in params:
        params[
            "linescaling"
        ] = 1  # Sets scaling relationship between adjacency matrix and line width
    ...
    for group in plot_groups:
        ...
        for xpos in xvals:
            all_nodes.append(
                go.Scatter(
                    x=xpos * np.ones(np.shape(plot_groups[group])),
                    y=ydat,
                    mode="markers",
                    showlegend=False,
                    marker=dict(
                        ...
                        line=dict(width=2, color="rgb(1,1,1)"),
                    ),
                )
            )
                ...
                all_lines.append(
                    go.Scatter(
                        ...
                        line=dict(
                            ...
                            color="rgba{}".format(
                                cl.to_rgba(colors[group], params["linealpha"])
                            ),
                        ),
                    )
                )
