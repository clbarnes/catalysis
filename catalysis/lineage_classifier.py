...
def lineage_table(hemilateral_groups, lin_landmarks, side_name):
    ...
    for lin in hemilateral_groups:
        ...
        if not any(slc):
            ...
            hemilateral_groups[lin]["l_loc"] = np.array(
                lin_landmarks[slc][["x_left", "y_left", "z_left"]]
            )
            hemilateral_groups[lin]["r_loc"] = np.array(
                lin_landmarks[slc][["x_right", "y_right", "z_right"]]
            )
...
def compute_initial_segment_dotprops(nrns, l_span, resample_distance):
    ...
    for nrn in nrns:
        ...
        initial_nodes = [
            nid for nid in nrn.minimal_paths()[0] if d_close[nrn.node2ind[nid]]
        ]
...
def compute_conditional_distributions(
    nblast_mat, lin_dict, lin_name2id, rel_lins, skip_skids=None
):
    """
    ...
    for lin in rel_lins:
        ...
        for skid1 in within_ids:
            all_sim = [
                nblast_mat[skid1][skid2]
                for skid2 in list(set(within_ids).difference(set([skid1])))
            ]
...
def lineage_likelihood_ratios(
    ...
    for ind, lin in enumerate(rel_lins):
        d_toA = np.median(
            np.array(
                [
                    nblast_mat[nblast_mat.index == rel_skid][skid].values
                    for skid in lin_dict[lin_name2id[lin]]
                ]
            )
        )
    ...
    return pd.DataFrame(
        {
            "Lineage": rel_lins,
            "LikelihoodRatio": lhds,
            "FractionOfBest": lhds_ratio,
            "SkeletonId": base_skid,
        }
    )
