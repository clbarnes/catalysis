...
def transform_neuronlist(
    ...
    """ """
    from_landmarks, to_landmarks = _parse_landmarks(
    ...
    for nrn in tqdm.tqdm(nrns_t):
        nrns_t.neurons[nrn.id] = transform_neuron_from_landmarks(
            nrn, from_landmarks=from_landmarks, to_landmarks=to_landmarks
        )
    return nrns_t


def transform_neuron_from_landmarks(
    nrn,
    from_group=None,
    to_group=None,
    CatmaidInterface=None,
    ...
    if transform_synapses:
        ...
        vinprimes = moving_least_squares_affine_vectorized(
            vins, from_landmarks, to_landmarks
        )
        ...
        voutprimes = moving_least_squares_affine_vectorized(
            vouts, from_landmarks, to_landmarks
        )
...
def landmark_group_list(CatmaidInterface):
    ...
    groups_raw = CatmaidInterface.get_landmark_groups(
        with_locations=True, with_members=False
    )
...
def url_to_transformed_point(
    ...
    xyzt = moving_least_squares_affine_from_landmarks(
        xyz0, from_landmarks, to_landmarks, ws
    )
...
def _parse_landmarks(
    ...
    if from_group is not None and to_group is not None and CatmaidInterface is not None:
        ...
        from_landmarks, to_landmarks = landmark_points(
            landmarks[from_group], landmarks[to_group], CatmaidInterface
        )
